...
(function(root, factory) {
	...
	var ngModule = angular.module("angular-oauth2", [ "ngStorage", "base64" ]).config(oauthConfig).factory("oauthInterceptor", oauthInterceptor).provider("OAuth", OAuthProvider).provider("OAuthToken", OAuthTokenProvider);
	...
	function OAuthProvider() {
		...
		this.$get = function($http, OAuthToken, $base64, $location) {
			var OAuth = function() {
				function OAuth() {
					if (!config) {
				...
				_prototypeProperties(OAuth, null, {
					...
					getAccessTokenFromCode: {
						value: function getAccessTokenFromCode(options) {
							var code;
					...
					getAuthorizationCode: {
						value: function getAuthorizationCode(options) {
							var data = {
								client_id: config.clientId,
								response_type: "code",
								redirect_uri: '/'
							...
							window.location.href = config.authorizePath + '?' + data;
						},
						writable: true,
						enumerable: true,
						configurable: true
		...
		this.$get.$inject = [ "$http", "OAuthToken", "$base64", "$location" ];
	}
	var _prototypeProperties = function(child, staticProps, instanceProps) {
		if (staticProps) Object.defineProperties(child, staticProps);
		if (instanceProps) Object.defineProperties(child.prototype, instanceProps);
		...
		this.$get = function($localStorage) {
			var OAuthToken = function() {
				...
				_prototypeProperties(OAuthToken, null, {
					token: {
						set: function(data) {
							return $localStorage[config.name] = data;
						},
						get: function() {
