<div class="container-fluid">
	<div class="row-fluid">
		...
		<div id="newsfeed" class="bs-docs-example span12">
			<#list newses.entityList as a>
			...
			<div style="margin: 7px; padding:7px; -webkit-border-radius: 4px; margin-bottom: 10px;"> 
				...
				<div class="btn-group pull-right">
					<#if page &gt; 1>
						...
						<a class="btn disabled">${page} of ${newses.totalPage}</a>
					<#if page &lt; newses.totalPage>
						<a class="btn" href="<@s.url value="${request.servletPath}?max=${max}&page=${page + 1}"/>">&nbsp;<i class="icon-forward"></i></a>
						<a class="btn" href="<@s.url value="${request.servletPath}?max=${max}&page=${newses.totalPage}"/>">&nbsp;<i class="icon-fast-forward"></i></a>
					</#if>
				</div>
			</div>
		</div>
