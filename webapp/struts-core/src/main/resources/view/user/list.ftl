...
<form method="get" class="form-inline span12">
	...
	<a class="btn btn-primary pull-right" href="${request.servletPath}/add">New User</a>
</form>
</div>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<a class="btn confirm" title="Delete" href="${request.servletPath}/delete/${c.id!}" data-message="Are you sure want to delete ${c.name.first!} ${c.name.last!}?"><i class="icon-trash"></i></a>
			</td>
		</tr>
		<#assign no = no + 1>
		</#list>
...
<div class="btn-group pull-right">
	...
	<a class="btn" href="${request.servletPath}?max=${max}&page=${users.totalPage}">&nbsp;<i class="icon-fast-forward"></i></a>
	</#if>
</div>